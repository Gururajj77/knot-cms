...
export async function createSetupSession(env: Env): Promise<string> {
    ...
    await env.DB.prepare(`INSERT INTO connect_sessions (id, expires_at) VALUES (?, ?)`)
        .bind(id, expiresAt)
        .run()
    return id
}
...
export async function saveSetupSessionToken(
    ...
    await env.DB.prepare(
        `UPDATE connect_sessions SET source_access_token_enc = ? WHERE id = ? AND datetime(expires_at) > datetime('now')`
    )
        .bind(enc, sessionId)
        .run()
}
...
export async function getSetupSessionToken(env: Env, sessionId: string): Promise<string | null> {
    const row = await env.DB.prepare(
        `SELECT source_access_token_enc FROM connect_sessions WHERE id = ? AND datetime(expires_at) > datetime('now')`
    )
        .bind(sessionId)
